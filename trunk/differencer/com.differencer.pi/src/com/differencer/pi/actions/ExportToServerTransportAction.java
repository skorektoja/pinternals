...
public class ExportToServerTransportAction extends Action {
	...
	public ExportToServerTransportAction(HashMap<ConfigurationNode, ConfigurationNode> n, String t, String a,  Server r) {
		nodes = n;
...
//		}
	...
	public void run() {
		...
		String confirmText = "Please, confirm transport action for "  + nodes.values().size() + " file(s)";
		...
		if (MessageDialog.openConfirm(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(), "Export Transport", confirmText)) {
			Job job = new Job("Export Transport to " + rightURL) {
				@Override
				protected IStatus run(IProgressMonitor monitor) {
					try {
						...
						for (ConfigurationNode i: nodes.values()) {
							...
							nodesitto.put(i,i);
							Exporter.exportTransportToServer(nodesitto, transportDirectory, transportArchiveDirectory, right);
							monitor.worked(1);
